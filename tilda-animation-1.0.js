...
function t_animate__init() {
    ...
    t_animate__parseNumberText(animElemsNumber);
    setTimeout(function() {
    ...
    if (animElems.length) {
        for (var s = 0; s < animElems.length; s++) {
            ...
            if (animElems[s].curTopOffset < curElemTrigger) {
                ...
                if ($(animElems[s]).is('[data-animate-style="animatednumber"]')) {
...
function t_animate__parseNumberText(curElem) {
    ...
    curElem.each(function() {
        ...
        if (text.length > 0) {
            ...
            var number = removeNumberSpace.match(/\d+/g);

            if (number) {
                if (number.length == 2) {
                    var lengthToFix = number[1].length;
                    number = +number.join('.');
                    number = number.toFixed(lengthToFix);
                }

                $this.attr('data-animate-number-count', text);

                t_animate__changeNumberOnZero($this, removeNumberSpace, number);
...
function t_animate__changeNumberOnZero(el, removeNumberSpace, number) {
    var el;
    var textWithoutNumber;
    var testNumber = '' + number;

    if (testNumber.search(/\./g) != -1) {
        textWithoutNumber = removeNumberSpace.replace(/\d+\.\d+|\d+\,\d+/g, '<span class="t-animate__number">' + 0 + '</span>');
    } else {
        textWithoutNumber = removeNumberSpace.replace(/\d+/g, '<span class="t-animate__number">' + 0 + '</span>');
    }
...
function t_animate__animateNumbers(curElem) {
    curElem.each(function() {
        ...
        if (text) {
            ...
            if (number.length == 2) {
                decimalLength = number[1].length;
                number = +number.join('.');
                isFloat = true;
            }

            var numberWrapper = $this.find('.t-animate__number').text();

            $({
                animateCounter: numberWrapper
            }).animate({
                ...
                step: function(isFloat) {
                    ...
                    isComma ? $this.find('.t-animate__number').text(animationStep.replace(/\./g, ',')) : $this.find('.t-animate__number').text(animationStep);
                },
                complete: function() {
                    $this.html(text);
                    $this.find('span').each(function() {
...
function t_animate__setAnimationState(animGroupsBlocks, animChainsBlocks, animElems) {
    ...
    animElems.each(function() {
        ...
        if (curElemOffsetTop < (viewTop - 500)) {
            ...
            if (curElem.is('[data-animate-style="animatednumber"]')) {
...
function t_animate__assignOrderedElemsDelay(curBlock, sectionHeadDelay) {
    ...
    if ((elem1.length || elem2.length || elem3.length || elem4.length || elem5.length || elem6.length) && elem7.length) {
        animDelay = animDelay + elem7.attr("data-animate-delay") * 1;
    ...
    if ((elem1.length || elem2.length || elem3.length || elem4.length || elem5.length || elem6.length || elem7.length) && elem8.length) {
        animDelay = animDelay + elem8.attr("data-animate-delay") * 1;
    ...
    if ((elem1.length || elem2.length || elem3.length || elem4.length || elem5.length || elem6.length || elem7.length || elem8.length) && elem9.length) {
        animDelay = animDelay + elem9.attr("data-animate-delay") * 1;
...
function t_animate__setCustomAnimSettings(curElem, curElemOffsetTop, viewBottom) {
    ...
    var curElemStyle = curElem.attr("data-animate-style");
    var curElemDistance = curElem.attr("data-animate-distance");
    ...
    var curElemScale = curElem.attr("data-animate-scale");
    ...
    var curElemDelay = curElem.attr("data-animate-delay");
    ...
    var curElemDuration = curElem.attr("data-animate-duration");
...
function t_animate__detectElemTriggerOffset(curElem, viewBottom) {
    var curElemTriggerOffset = curElem.attr("data-animate-trigger-offset");
...
function t_animate__wrapTextWithOpacity() {
    var animTextElems = $(".t-title.t-animate,.t-descr.t-animate,.t-uptitle.t-animate,.t-text.t-animate");
    animTextElems.each(function() {
        var curElem = $(this),
            curElemStyle = $(this).attr("style");
        if (typeof curElemStyle != "undefined" && curElemStyle.indexOf("opacity") >= 0) {
            var curElemOpacityIndex = curElemStyle.indexOf("opacity");
            if (curElemStyle.slice(curElemOpacityIndex).indexOf(";") >= 0) {
                var curElemOpacity = curElemStyle.slice(curElemOpacityIndex, curElemOpacityIndex + curElemStyle.slice(curElemOpacityIndex).indexOf(";"));
            } else {
                var curElemOpacity = curElemStyle.slice(curElemOpacityIndex);
            }
            var curElemHtml = "<div style=\"" + curElemOpacity + ";\">" + curElem.html() + "</div>";
            curElem.css("opacity", "");
            curElem.html(curElemHtml);
        }
    });
}
